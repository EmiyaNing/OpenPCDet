...
from pcdet.models import build_network, model_fn_decorator, ema_model_fn_decorator
from pcdet.utils import common_utils
from train_utils.optimization import build_optimizer, build_scheduler
from train_utils.kd_train_utils import train_model_kd
from semi_utils.semi_train_utils import train_model_sess
...
def parse_config():
    ...
    parser.add_argument('--cfg_file', type=str, default=None, help='specify the student config for training')
    ...
    cfg_from_yaml_file(args.cfg_file, student_cfg)
    student_cfg.TAG = Path(args.cfg_file).stem
    student_cfg.EXP_GROUP_PATH = '/'.join(args.cfg_file.split('/')[1:-1])  # remove 'cfgs' and 'xxxx.yaml'



    return args, student_cfg


def main():
    args, student_cfg = parse_config()
    ...
    train_set, train_loader, train_sampler = build_semi_dataloader(
        dataset_cfg=student_cfg.DATA_CONFIG,
        class_names=student_cfg.CLASS_NAMES,
        batch_size=args.batch_size,
        dist=dist_train,
        root_path=student_cfg.DATA_CONFIG.DATA_PATH,
    ...
    model.set_model_type('student')
    model_ema.set_model_type('teacher')

    for params in model_ema.parameters():
        params.detach_()
