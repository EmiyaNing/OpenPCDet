"""
    The Voxel_TASubHeads based on the voxel_subhead_simplev2.
    In this version self_distillation framework, we only distillation the target class of selected student target and teacher target..
"""
import math
import torch
...
class Voxel_TASubHeads(Detector3DTemplate):
    ...
    def get_main_roihead_self_distillation_loss(self):
        ...
        one_hot_targets = one_hot_targets[:, 1:].bool() #Nt, 3
        ...
        matched_student_cls_preds = stage_one_cls[student_index_of_teacher] # keep the shape consistency of teacher and student

        student_target_class_pred = matched_student_cls_preds[one_hot_targets].unsqueeze(-1) # [Nt, 1]


        cls_loss = self.distill_cost_function(student_target_class_pred.log(), stage_two_cls) # student_cls_preds [Nt, 1], teacher_cls_preds [Nt, 1]
