"""
    In this version, we use a bigger thresh for self distillation.
"""
import math
import torch
...
class Voxel_PerSubHeadsv4(Detector3DTemplate):
    ...
    def get_training_loss(self):
        ...
        if not self.model_cfg.SELFKD:
            ...
            if self.model_cfg.get('RELATION', None) is not None:
                loss_relation, tb_relation_dict = self.get_main_roihead_self_relation_distillation_loss()
                loss += loss_relation * self.model_cfg.SELFWEIGHT
                tb_dict.update(tb_relation_dict)
    ...
    def get_main_roihead_self_distillation_loss(self):
        ...
        for bs_idx in range(batch_sz):
            ...
            cls_loss_temp = self.distill_cost_function(matched_student_cls_preds.log(), one_hot_targets)
            cls_loss      += (cls_loss_temp * matched_student_mask).sum() / num_teacher_box 
            batch_normize += 1

        cls_loss = cls_loss / batch_normize
    ...
    def get_main_roihead_self_relation_distillation_loss(self):
        stage_one_cls = F.sigmoid(self.forward_ret_dict['stage_one_cls'])
        stage_one_box = self.forward_ret_dict['stage_one_box']
        stage_two_cls = F.sigmoid(self.forward_ret_dict['main_stage_two_cls'])
        stage_two_box = self.forward_ret_dict['main_stage_two_box']
