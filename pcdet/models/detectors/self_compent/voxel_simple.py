'''
    ...
    change the matched rule....From match student to matche teacher.
'''
import math
import torch
...
class Voxel_Simple(Detector3DTemplate):
    ...
    def get_main_roihead_self_distillation_loss(self):
        ...
        with torch.no_grad():
            ...
            matched_teacher_box  = matched_teacher_mask.squeeze(-1).sum()

        matched_teacher_cls_preds = one_hot_targets[teacher_index_of_student] # duiqi shape with teacher
        cls_loss = self.distill_cost_function(stage_one_cls.log(), matched_teacher_cls_preds)
        cls_loss = (cls_loss * matched_teacher_mask).sum() / (num_student_box * batch_sz)
        ...
        tb_dict = {
            ...
            'matched-student-boxes': matched_teacher_box.item(),
        } 
