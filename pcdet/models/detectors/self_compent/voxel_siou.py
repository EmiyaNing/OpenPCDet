'''
    Using the same setting with voxel_subhead_simplev2.py
    In this version, we add the iou loss into the self distillation as a regularize...
'''
import math
import torch
...
class Voxel_Siou(Detector3DTemplate):
    ...
    def get_main_roihead_self_distillation_loss(self):
        ...
        with torch.no_grad():
            ...
            matched_student_box  = matched_student_mask.squeeze(-1).sum().item()

        matched_student_cls_preds = stage_one_cls[student_index_of_teacher] # duiqi shape with teacher
        matched_student_box_preds = stage_one_box[student_index_of_teacher]
        ...
        iou_ground = boxes_iou3d_gpu(matched_student_box_preds, stage_two_box) # [Ns, Ns]
        iou_ground = torch.max(iou_ground, dim=-1, keepdim=False)[0]
        iou_student= iou_ground * matched_student_mask.squeeze(-1)

        with torch.no_grad():
            positive_box_count = (iou_student > 0.7).float().sum().item()

        iou_loss   = (1 - iou_student).sum() / (num_student_box * batch_sz)



        cls_loss = self.distill_cost_function(matched_student_cls_preds.log(), one_hot_targets)
        cls_loss = (cls_loss * matched_student_mask).sum() / (num_student_box * batch_sz)

        siou_loss = cls_loss + iou_loss * self.model_cfg.IOU_WEIGHT


        self.sub_consistency_meter.update(siou_loss.item())
        tb_dict = {
            'Self-kd-subheads-loss': siou_loss.item(),
            'Self-kd-subheads-cls-loss': cls_loss.item(),
            'Self-kd-subheads-iou-loss': iou_loss.item(),
            'Self-kd-subheads-mean-loss': self.sub_consistency_meter.avg,
            'fitler-teacher-boxes': num_teacher_box,
            'filter-student-boxes': num_student_box,
            'matched-student-boxes': matched_student_box,
            'positive_box_count': positive_box_count,
        } 
        
        return siou_loss, tb_dict
