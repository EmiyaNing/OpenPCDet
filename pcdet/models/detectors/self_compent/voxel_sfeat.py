...
class Voxel_Feat(Detector3DTemplate):
    ...
    def forward(self, batch_dict):
        ...
        if self.training:
            ...
            self.forward_ret_dict['shallower_feat']= batch_dict['spatial_features']
            self.forward_ret_dict['deeper_feat']   = batch_dict['spatial_features_2d']
    ...
    def get_training_loss(self):
        ...
        if not self.model_cfg.SELFKD:
            ...
            if self.model_cfg.get('FEAT_DISTILL', None) is not None:
                loss_feat, tb_feat_dict = self.get_self_feature_loss()
                loss += loss_feat * self.model_cfg.FEAT_DISTILL
                tb_dict.update(tb_feat_dict)
    ...
    def get_self_feature_loss(self):
        cost_func      = nn.MSELoss()
        shallower_feat = self.forward_ret_dict['shallower_feat'].mean(dim=1)
        deeper_feat    = self.forward_ret_dict['deeper_feat'].mean(dim=1)

        feat_loss      = cost_func(shallower_feat, deeper_feat)
        tb_dict        = {
            'feature-loss': feat_loss.item(),
        }
        return feat_loss, tb_dict
