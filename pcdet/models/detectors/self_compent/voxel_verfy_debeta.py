...
class Voxel_VDEBETA(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.pt_weight         = model_cfg.PT_WEIGHT
        self.pnt_weight        = model_cfg.PNT_WEIGHT
    ...
    def get_main_roihead_self_distillation_loss(self):
        ...
        linear_rise_weight = ((1 - stage_two_cls) / (self.num_class - 1)) * self.beta
        ...
        stage_two_hot = torch.zeros(
            *list(stage_two_cls.squeeze(-1).shape), self.num_class + 1, dtype=stage_two_cls.dtype, device=stage_two_cls.device
        )
        stage_two_hot.scatter_(-1, stage_two_label.unsqueeze(dim=-1).long(), stage_two_cls)
        stage_two_hot = stage_two_hot[:, 1:].float()

        zero_mask     = stage_two_hot == 0
        stage_two_hot[zero_mask] = linear_rise_weight.repeat(1, self.num_class - 1).reshape(-1)
        ...
        with torch.no_grad():
            ...
            positive_count = (matched_student_mask > 0).sum()
        ...
        pnt_tea          = stage_two_hot[one_hot_targets.logical_not()]

        pnt_tea          = pnt_tea.reshape(num_teacher_box, self.num_class - 1).sum(-1)
        pnt_stu          = pnt_stu.reshape(num_teacher_box, self.num_class - 1).sum(-1)

        tckd_pt_part     = (self.distill_cost_function(pt_stu.log(), stage_two_cls) * matched_student_mask).sum()
        tckd_pnt_part    = (self.distill_cost_function(pnt_stu.log(), pnt_tea) * matched_student_mask).sum()
        tckd_loss        = tckd_pt_part * self.pt_weight + tckd_pnt_part * self.pnt_weight
        ...
        tb_dict = {
            ...
            'Self-kd-pt-part': tckd_pt_part.item() / (num_teacher_box * batch_sz),
            'Self-kd-pnt-part': tckd_pnt_part.item() / (num_teacher_box * batch_sz),
            'total_teacher_box': num_teacher_box,
            'positive_box_count': positive_count.item(),
        }   
