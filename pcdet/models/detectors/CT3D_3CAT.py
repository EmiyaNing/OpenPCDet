...
class CT3D_Teacher(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        super().__init__(model_cfg=model_cfg, num_class=num_class, dataset=dataset)
        self.module_list = self.build_networks()
    ...
    def forward(self, batch_dict):
        ...
        batch_dict['stage_one_box'] = copy.deepcopy(batch_dict['batch_box_preds'])
        batch_dict['stage_one_cls'] = copy.deepcopy(batch_dict['batch_cls_preds'])
        batch_dict['backbone_bev_feat'] = copy.deepcopy(batch_dict['spatial_features'])
        batch_dict['backbone_2d_feat'] = copy.deepcopy(batch_dict['spatial_features_2d'])
