...
class odiou_3D(nn.Module):
    ...
    def forward(ctx, gboxes, qboxes, weights, batch_size):
        ...
        #remove the nan value from the corners_gboxes....
        zeros_tensor   = torch.zeros(corners_gboxes.shape).cuda()
        nan_index      = torch.isnan(corners_gboxes)
        corners_gboxes[nan_index] = zeros_tensor[nan_index]
